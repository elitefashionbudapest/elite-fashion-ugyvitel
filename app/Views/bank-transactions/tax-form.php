...
<div class="max-w-lg">
    <div class="bg-surface-container-lowest rounded-xl p-6 sm:p-8">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h3 class="font-heading font-bold text-on-surface text-xl">Adó kifizetés</h3>
                <p class="text-xs text-on-surface-variant">ÁFA, SZJA, TB, egyéb adók és járulékok kifizetése.</p>
            </div>
        </div>

        <form method="POST" action="<?= base_url('/bank-transactions/tax') ?>" class="space-y-5">
            <?= csrf_field() ?>
            ...
            <div>
                <label class="block text-xs font-bold text-yellow-600 uppercase tracking-widest mb-1.5">Összeg</label>
                <div class="relative">
                    <input type="text" inputmode="decimal" data-calc name="amount" step="0.01" min="0" value="<?= e(old('amount')) ?>"
                           class="w-full px-4 py-3 border border-yellow-300 rounded-xl text-lg font-heading font-bold text-center focus:ring-2 focus:ring-yellow-200 focus:border-yellow-400 bg-yellow-50"
                           placeholder="0" required>
                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-yellow-400">Ft</span>
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1.5">Megjegyzés (opcionális)</label>
                <input type="text" name="notes" value="<?= e(old('notes')) ?>" class="<?= $inputCls ?>" placeholder="pl. ÁFA 2026/Q1, SZJA március">
            </div>
