    ...
    <div class="bg-surface-container-lowest rounded-xl p-6 sm:p-8">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                ...
                <p class="text-xs text-on-surface-variant">Tagi kölcsön befizetése a cég bankszámlájára.</p>
            </div>
        </div>
        ...
        <form method="POST" action="<?= base_url('/bank-transactions/owner-loan') ?>" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1.5">Melyik bankszámlára?</label>
            ...
            <div>
                <label class="block text-xs font-bold text-emerald-600 uppercase tracking-widest mb-1.5">Kölcsön összege</label>
                <div class="relative">
                    <input type="number" name="amount" step="0.01" min="0" value="<?= e(old('amount')) ?>"
                           class="w-full px-4 py-3 border border-emerald-300 rounded-xl text-lg font-heading font-bold text-center focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400 bg-emerald-50"
                           placeholder="0" required>
                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-emerald-400">Ft</span>
                </div>
            </div>
