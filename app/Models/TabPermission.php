...
class TabPermission
{
    ...
    /**
     * Egy felhasználó egy tab jogosultsága
     */
    public static function getPermission(int $userId, string $tabSlug): ?array
    {
        $db = Database::getInstance();
        $stmt = $db->prepare(
            'SELECT * FROM tab_permissions WHERE user_id = :user_id AND tab_slug = :tab_slug'
        );
    ...
    /**
     * Egy felhasználó összes tab jogosultsága
     */
    public static function getAllForUser(int $userId): array
    {
        $db = Database::getInstance();
        $stmt = $db->prepare(
            'SELECT * FROM tab_permissions WHERE user_id = :user_id ORDER BY tab_slug'
        );
    ...
    /**
     * Tab jogosultság mentése (upsert)
     */
    public static function save(int $userId, string $tabSlug, bool $canView, bool $canEdit): void
    {
        $db = Database::getInstance();
        $stmt = $db->prepare(
            'INSERT INTO tab_permissions (user_id, tab_slug, can_view, can_edit)
             VALUES (:user_id, :tab_slug, :can_view, :can_edit)
             ON DUPLICATE KEY UPDATE can_view = :can_view2, can_edit = :can_edit2'
        );
        $stmt->execute([
            'user_id'   => $userId,
            'tab_slug'  => $tabSlug,
    ...
    /**
     * Van-e beállított jogosultsága a felhasználónak?
     * Ha nincs (pl. Ádám), akkor korlátlan hozzáférés.
     */
    ...
    /**
     * Felhasználó látható tabjai (sidebar-hoz)
     */
    public static function getVisibleTabs(int $userId): array
    {
        $db = Database::getInstance();
        $stmt = $db->prepare(
            'SELECT tab_slug FROM tab_permissions WHERE user_id = :user_id AND can_view = 1'
        );
        $stmt->execute(['user_id' => $userId]);
    ...
    /**
     * Ellenőrzés: van-e szerkesztési joga
     */
